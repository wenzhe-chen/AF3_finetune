...
def main(configs: Any) -> None:
    ...
    runname = configs.load_checkpoint_path.split('/')[-1].split('.')[0]
    ...
    feats = np.concatenate(logits, axis=0)
    ...
    plt.figure()
    # Binary ROC
    fpr, tpr, _ = roc_curve(labels, feats[:,1])
    roc_auc = auc(fpr, tpr)
    ...
    plt.tight_layout()
    plt.savefig('./output/roc_curve_'+runname+'.png')

    # Save ROC data for later plotting
    np.savez(
        f'./output/roc_curve_{runname}_data.npz',
        fpr=fpr,
        tpr=tpr,
        roc_auc=roc_auc
    )
